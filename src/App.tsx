  ...
  return <main className="game-shell">
    ...
    <div className="game-layout">
      ...
      >
        ...
        {phaseOne && (props.event === 'p1-memory-position' || props.event === 'p1-memory-sweep') && <div className="rune-order p1-rune-order" role="status"><span>MEMORY ORDER</span><div className="p1-rune-grid">{props.p1MemoryOrder.map((rune, index) => <strong className={`${props.p1FailedMemoryRune === rune ? 'failed' : props.event === 'p1-memory-sweep' && index <= Math.floor(props.eventTime / P1_MEMORY_SWEEP_SECONDS * 5) ? 'active' : ''}${rune === playerP1Rune ? ' personal' : ''}`} key={`${rune}-${index}`}><small>{index + 1}</small><b>{rune}</b></strong>)}</div></div>}
