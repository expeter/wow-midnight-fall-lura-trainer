...
describe('player menu', () => {
  ...
  it('derives crystal duty from the phase roster and starts with a countdown', async () => { const user = userEvent.setup(); renderRaidPlan(); fireEvent.change(screen.getByLabelText(/assignment position/i), { target: { value: '1' } }); expect(screen.getByLabelText(/intermission crystal 1/i)).toHaveValue('1'); await user.click(screen.getByRole('button', { name: /enter arena/i })); expect(screen.getByText(/Get ready/i)).toBeInTheDocument(); expect(screen.getByText('3')).toBeInTheDocument(); expect(screen.getByText(/CRYSTAL CARRIER/i)).toBeInTheDocument(); expect(screen.getByText(/You received a crystal · Intermission/i)).toBeInTheDocument(); expect(screen.getByText('The veil shudders with every step.')).toBeInTheDocument(); expect(screen.queryByText(/MAIN ABILITY READY/i)).not.toBeInTheDocument(); expect(screen.getByText('Points')).toBeInTheDocument(); expect(document.querySelector('.hud')).not.toBeInTheDocument() })
  ...
  it('uses shallow setup tabs for common settings, raid plans, and online features', () => {
    render(<App />)
    ...
    const ordered = [gameHeading, difficulty, keyboardHeading, input, hudHeading, hud]
