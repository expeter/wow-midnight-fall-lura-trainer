...
| `FR-030` | Implemented behind localhost preview | Add the five-interrupt, crystal pickup, ricocheting glaive, ordered rune beam, rotating-beam/reactive-Soak encounter, and fifteen-second Intermission handoff before the existing encounter. Includes a dedicated persisted P1 raid plan, six crystal assignments, Numpad 2 interrupt binding, renderer integration, TTS calls, and focused regression coverage. Preserve the mechanic contract in [`p1-encounter.md`](p1-encounter.md). |
...
| `CR-127` | Implemented behind localhost preview | During the fifteen-second P1 handoff, move L’ura smoothly from her final outside stop into the Intermission center rather than teleporting at the phase boundary. Tracked from [`INBOX-20260727-214015-ef8287`](../inbox/INBOX-20260727-214015-ef8287.md). |
...
| `CR-149` | Implemented | Remove the encrypted API backup certificate and GitHub Actions artifact workflow from the repository. Keep the consistent rotating VPS-local SQLite backup timer, and leave off-VPS replication pending until the user provides a separate storage destination. |
...
| `CR-152` | Implemented | Separate signed-in character/profile management from the public leaderboard, explain what profile visibility settings and leaderboard rows do, clarify search/filter controls, and style the online controls consistently with the trainer. |
...
| `CR-156` | Planned | During the Phase 4 safe-spot mechanic, being outside the safe area continuously drains player health and applies negative points, using the existing damage/score contract rather than changing safe-spot geometry or timing. Tracked from [`INBOX-20260729-151750-6795a1`](../inbox/INBOX-20260729-151750-6795a1.md). |
| `CR-157` | Planned | Add a localhost-only finish-screen submission fixture that uses a server-issued synthetic attempt and the real completion/refresh UI, allowing verified point submission to be tested without playing the encounter. Keep it unavailable in production and visibly label the result as test data. |
| `FR-058` | Planned | Add an achievement Hall of Fame. Assign explicit difficulty weights to achievements, rank players by total weighted achievement points, and show each player’s highest-value achievement plus its first-earned time so completionists can build a durable historical ranking. |
