...
import { bossDamageScoreBonus, isInsideP3Pool, p4BossHealthWithPlayerDamage, p4FrontSoakerPosition, p4StartingBossState, p4TankKillsBox, preP4BossHealth, shouldTriggerP3EarlyClear, starsplinterHitsCrystalCarrier } from './game'
...
export default function App() {
  ...
  const [p2OrbReturnSeed, setP2OrbReturnSeed] = useState(() => Math.floor(Math.random() * 2147483647))
  ...
  const p2OrbReturnSeedRef = useRef(p2OrbReturnSeed)
  ...
  const initializeAttempt = (preserveScore = false) => {
    ...
    const nextP2OrbReturnSeed = Math.floor(Math.random() * 2147483647)
    p2OrbReturnSeedRef.current = nextP2OrbReturnSeed
    setP2OrbReturnSeed(nextP2OrbReturnSeed)
    ...
    const tick = (now: number) => {
      ...
      if (p2OrbReturnAgeRef.current >= 0 && p2OrbReturnAgeRef.current < P2_NEXT_BEAM_AFTER_RESOLUTION_SECONDS + .5) {
        ...
        if (nextOrbAge >= returnFlightStarts && nextOrbAge < returnFlightEnds && !p2OrbPlayerHitRef.current) {
          const returningOrbs = p2ReturningOrbPositions(nextOrbAge, p2OrbReturnSeedRef.current, WORLD.center)
  ...
  }, [screen, paused, movementSpeed, movementBonus, gameSpeed, event, beamAngles, npcSplinters, crystal, npcCrystals, keyBindings, difficulty, healthPotEnabled, shieldEnabled, assignment, p3Round, p3RuneOrder, p3ResolvedRunes, p3Positions, p4PatternSeed])

  useEffect(() => {
    if (screen !== 'game' || paused) return
    const duration = event === 'positioning' ? 10 : event === 'p1-recover' ? P1_FINAL_RECOVERY_SECONDS : event === 'p2-jump' ? 1.4 : event === 'p2-positioning' ? P2_POSITIONING_SECONDS : event === 'p2-orbs' ? P2_BEAM_SECONDS : event === 'p2-recover' || event === 'p2-fetch' ? crystal ? Infinity : 0 : event === 'p2-spread' ? P2_SPREAD_SECONDS : event === 'p2-pull' ? P2_PULL_SECONDS : event === 'p2-wait' ? Infinity : event === 'p3-flight' ? P3_FLIGHT_SECONDS : event === 'p3-landing' ? 3 : event === 'p3-approach' ? P3_APPROACH_SECONDS : event === 'p3-light-pools' ? P3_SECTOR_SECONDS : event === 'p3-pools-overlap' ? 15 : event === 'p3-rune-preview' ? 2 : event === 'p3-lattice-memory' ? 10 : event === 'p3-lattice-second' ? 4.5 : event === 'p3-big-boom' ? 1 : event === 'p3-archangel-position' ? 4 : event === 'p3-archangel' ? 6 : event === 'p3-sector-move' ? p3Round >= 2 ? P3_FINAL_SECTOR_MOVE_SECONDS : 5 : event === 'p4-countdown' ? 3 : event === 'p4-transition' ? P4_KNOCKUP_SECONDS : event === 'p4-cycle' ? P4_CYCLE_SECONDS : 3
    ...
    if (event === 'p4-countdown') {
      ...
      p2OrbReturnHitRef.current = false
      p2OrbPlayerHitRef.current = false
      const nextReturnSeed = Math.floor(Math.random() * 2147483647)
      p2OrbReturnSeedRef.current = nextReturnSeed
      setP2OrbReturnSeed(nextReturnSeed)
      setP2OrbReturnAge(0)
    ...
    <div className="game-layout">
      ...
      >
        <GameScene
          ...
          p2OrbReturnSeed={props.p2OrbReturnSeed}
        ...
        <div className={`splinter-counter${phaseFour ? ' p4-timers' : phaseThree && props.role === 'carrier' ? ' p3-duty-counter' : ''}`} style={{ left: `${props.hudLayout.mechanic.x}%`, top: `${props.hudLayout.mechanic.y}%` }}>
          {phaseFour
            ...
            : p3
              ...
              : p2
                ? <>{p2PhaseTransitionRemaining !== null ? 'PHASE 3' : p2.counter} <strong>{props.event === 'p2-recover' || props.event === 'p2-fetch' ? Math.max(0, 6 - props.crystalAge).toFixed(1) : props.event === 'p2-wait' ? (p2PhaseTransitionRemaining ?? Math.max(0, P2_NEXT_BEAM_AFTER_RESOLUTION_SECONDS - props.p2OrbReturnAge)).toFixed(1) : Math.max(0, p2.duration - props.eventTime).toFixed(1)}s</strong></>
                : countdown
                  ? <>STARTING <strong>{Math.max(0, 3 - props.eventTime).toFixed(1)}s</strong></>
                  : positioning
