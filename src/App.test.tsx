...
describe('player menu', () => {
  ...
  it('teases P1 before Intermission without making it playable', () => {
    render(<App />)
    const p1 = screen.getByRole('button', { name: /p1.*coming soon/i })
    const intermission = screen.getByRole('button', { name: /^intermission$/i })
    expect(p1).toBeDisabled()
    expect(p1.compareDocumentPosition(intermission) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
  })
  it('shows the version, Git revision, changelog, and build time on setup and in the arena', async () => { const user = userEvent.setup(); render(<App />); const build = screen.getByLabelText(/build information/i); expect(build).toHaveTextContent(/^v0\.1\.0 · (?:unknown|[0-9a-f]{7}) · \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC · GitHub ↗Changelog ↗$/); expect(screen.getByRole('link', { name: /v0\.1\.0.*github/i })).toHaveAttribute('href', 'https://github.com/expeter/wow-midnight-fall-lura-trainer'); expect(screen.getByRole('link', { name: /changelog/i })).toHaveAttribute('href', 'https://github.com/expeter/wow-midnight-fall-lura-trainer/blob/main/CHANGELOG.md'); await user.click(screen.getByRole('button', { name: /enter arena/i })); expect(screen.getByLabelText(/build information/i)).toBeInTheDocument() })
  ...
  it('uses a consistent setup hierarchy for game, input, HUD, sharing, and phase plans', () => {
    ...
    const combat = screen.getByRole('group', { name: /optional combat actions/i })
  ...
  it('keeps the opening boost and Main ability permanent while disabling health reactions on Easy', async () => { localStorage.setItem('lura-opening-speed-bonus', 'false'); const user = userEvent.setup(); render(<App />); expect(screen.queryByLabelText(/opening movement bonus/i)).not.toBeInTheDocument(); expect(screen.queryByLabelText(/enable main ability/i)).not.toBeInTheDocument(); expect(screen.getByText(/40% opening movement boost is always active/i)).toBeInTheDocument(); expect(screen.getByText(/Main ability · F/i)).toBeInTheDocument(); await waitFor(() => expect(localStorage.getItem('lura-opening-speed-bonus')).toBeNull()); await user.click(screen.getByRole('button', { name: 'easy' })); expect(screen.getByLabelText(/enable health potion/i)).toBeDisabled(); expect(screen.getByLabelText(/enable shield/i)).toBeDisabled() })
  it('rebinds gameplay actions and displays the optional health HUD', async () => { const user = userEvent.setup(); render(<App />); const forward = screen.getByLabelText(/forward keybind/i); fireEvent.keyDown(forward, { code: 'KeyI', key: 'i' }); expect(forward).toHaveValue('I'); await waitFor(() => expect(JSON.parse(localStorage.getItem('lura-keybindings') || '{}').forward).toBe('KeyI')); await user.click(screen.getByLabelText(/enable health potion/i)); await user.click(screen.getByRole('button', { name: /enter arena/i })); expect(screen.getByText(/100%/i)).toBeInTheDocument(); expect(screen.getByText(/Num Del POT/i)).toBeInTheDocument() })
  ...
  it('keeps valid southern P3 assignments and explains group selection', async () => { const saved = Array.from({ length: 20 }, (_, index) => ({ x: index < 10 ? 420 : 540, y: 380 })); localStorage.setItem('lura-p3-player-positions', JSON.stringify(saved)); const user = userEvent.setup(); render(<App />); expect(screen.getByText(/drag empty space to select a group/i)).toBeInTheDocument(); await user.click(screen.getByRole('button', { name: /save layout/i })); const positions = JSON.parse(localStorage.getItem('lura-p3-player-positions') || '[]'); expect(positions[0].y).toBeGreaterThan(270); expect(positions[10].y).toBeGreaterThan(270) })
  ...
  it('completes the always-available Main ability before awarding points and firing', async () => { const user = userEvent.setup(); render(<App />); await user.click(screen.getByRole('button', { name: /enter arena/i })); fireEvent.keyDown(window, { code: 'KeyF', key: 'f' }); expect(screen.getByText(/MAIN ABILITY · 1\.0s/i)).toBeInTheDocument(); expect(screen.getByText(/L’URA · 100\.0%/i)).toBeInTheDocument(); expect(screen.getByText('Points').nextElementSibling).toHaveTextContent('1000'); fireEvent.keyDown(window, { code: 'KeyF', key: 'f' }); expect(screen.getByText('Points').nextElementSibling).toHaveTextContent('1000'); await waitFor(() => expect(screen.getByText(/L’URA · 99\.5%/i)).toBeInTheDocument(), { timeout: 1800 }); expect(screen.getByText('Points').nextElementSibling).toHaveTextContent('1001') })
