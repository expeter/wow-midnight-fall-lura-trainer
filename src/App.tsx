  ...
  return <main className="game-shell">
    ...
    <div className="game-layout">
      ...
      >
        ...
        {phaseOne && (props.event === 'p1-memory-position' || props.event === 'p1-memory-sweep') && <div className="rune-order p1-rune-order" role="status"><span>MEMORY ORDER</span>{props.p1MemoryOrder.map((rune, index) => <strong className={props.p1FailedMemoryRune === rune ? 'failed' : props.event === 'p1-memory-sweep' && index <= Math.floor(props.eventTime / P1_MEMORY_SWEEP_SECONDS * 5) ? 'active' : ''} key={`${rune}-${index}`}>{rune}</strong>)}</div>}
