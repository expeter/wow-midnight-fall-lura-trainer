...
export default function App() {
  ...
  if (screen === 'menu') return <main className="shell setup-shell">
    ...
    <div className="entry-choice"><span>Practice target</span><button className={entryMode === 'arena1' ? 'selected' : ''} onClick={() => setEntryMode('arena1')}>Arena 1 → Arena 2</button><button className={entryMode === 'arena2' ? 'selected' : ''} onClick={() => setEntryMode('arena2')}>Arena 2 only</button><button className="coming-soon" disabled>Phase 3 · Coming soon</button><button className="coming-soon" disabled>Phase 4 · Coming soon</button><button className="start entry-start" onClick={start}>Enter {entryMode === 'arena2' ? 'Arena 2' : 'Arena 1'}</button></div>
