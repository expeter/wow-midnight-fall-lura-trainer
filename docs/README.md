...
| `FR-068` | In progress | Add the optional two-player tank role defined by [`tank-mechanic.md`](tank-mechanic.md) to the current encounter-completion milestone. Two explicitly configured raid-plan assignments handle the Heaven's Lance counter, full five-hit mitigation, post-burst Taunt/Spott swap, independent Impaled stacks, crystal combinations, and two-strike failure rule during boss-active P1, P2, and P3; assigned NPC tanks execute it correctly. A controlled tank receives a faster continuously recharging shield, can simultaneously have crystal duty, earns tank-role achievements, and in P4 plays the existing frontal cone instead of Starsplinter. Intermission suspends the tank loop. |
