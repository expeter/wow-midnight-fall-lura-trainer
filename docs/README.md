...
| `CR-165` | Backlog | Show the recorded wipe reason in each wipe activity-feed entry in addition to its phase and difficulty, preserving the existing privacy-aware identity treatment. |
| `CR-166` | Backlog | Allow a controlled crystal carrier to drop a carried crystal at any time that carrying is valid, including after a Phase 1 pickup; dropping starts the normal explosion countdown and the player may recollect it without the drop/recollection itself counting as a failure. |
| `CR-167` | Backlog | Keep the small yellow carried-crystal glow indicator visible on controlled crystal carriers whenever they actually carry a crystal, including Phase 2, so pickup state remains readable while L’ura occupies the center and after circle spreads. |
...
| `BUG-113` | Backlog | Prevent a full-pull Normal attempt from reporting “Entered the void zone” during Phase 2 or the Phase 2-to-Phase 3 transition when the player remains in the valid encounter arena. Diagnose the exact boundary/state transition and keep rendering, collision, and the documented arena contract aligned. |
| `BUG-114` | Backlog | During Intermission, stop controlled-player and NPC cosmetic Main-ability projectiles from targeting L’ura's former Phase 1 position; use the authoritative current boss position without changing damage, scoring, or encounter timing. |
