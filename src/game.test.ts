...
import { angleToward, ARENA, assignmentRevealDistance, bossBeamHitsPlayer, canPickupCrystal, canRecoverFromWipe, crystalCarrierPosition, crystalWipeReason, difficultySettings, distance, distanceToSegment, healthResponsesPerPhase, INTERMISSION_SEQUENCE, isInP3ConsumedSector, isP3ConsumedSectorLethal, isP3RuneTurn, isInSafeAnnulus, isInsideArena, isProtectedByP3Bubble, jumpHeights, moveInBounds, movePlayer, moveRelativeToCamera, moveWithIncreasingPull, nearestRuneEdges, npcEntryPosition, OPENING_BOOST_SECONDS, orientedAssignments, p2NpcCrystalDrops, p2OrbReturnState, p2ReturningOrbPositions, p3ArchangelStackPosition, p3AssignmentForRound, p3BossPosition, p3LandingGroupCenter, p3LandingGroupIndex, p3LandingPosition, p3LandingSoakPositions, p3LightCenters, p3LightHealthRate, p3NpcPoolAssignment, p3NpcSoaksActive, p3PoolCenters, p3PoolSoakRate, p3RuneDeadline, p3RuneOrbs, P3_RUNE_ORB_MIN_GAP, p3StarsTiming, p3WrongRuneContact, P3_OUTER_RADIUS, P3_POOL_HEALTH, P3_POOL_RADIUS, P3_SECOND_SOAK_NPC_DELAY_SECONDS, p4BossHealth, p4BoxStates, p4EncounterBoxStates, p4FrontSoakerPosition, p4GroupPosition, p4NpcRelocationPace, p4NpcSplinterPosition, p4PlayerSplinterDuty, p4RelocationProgress, p4SplinterAge, p4SplinterHitsGroup, p4SplinterResolutionActive, p4SplinterRotation, p4SplinterStartSeconds, p4StackPosition, p4TankConeActive, P4_BOX_COUNT, P4_BOX_MIN_SEPARATION, P4_BOX_SPEED, P4_CYCLE_SECONDS, P4_HEAVEN_START_SECONDS, P4_KNOCKUP_SECONDS, P4_PROTECTION_RADIUS, P4_SPLINTER_DETONATION_SECONDS, P4_SPLINTER_INTERVAL_SECONDS, P4_TANK_CONE_DURATION_SECONDS, P4_TANK_CONE_INTERVAL_SECONDS, personalCircleHitsCrystal, PLAYER_COLLISION_PENALTY, randomCrystalDropDuty, randomizeP3PoolLayout, roamingNpcPosition, seededStars, setP3BossPlan, translateSelectedPoints, WIPE_PENALTY } from './game'
...
describe('Intermission game rules', () => {
  ...
  it('lands the raid in six compact groups spread around the full Phase 3 circle', () => { const center = { x: 480, y: 270 }; const centers = Array.from({ length: 20 }, (_, index) => p3LandingGroupCenter(index, center)); expect(P3_OUTER_RADIUS).toBe(199); expect(new Set(centers.map(point => `${point.x.toFixed(2)}:${point.y.toFixed(2)}`)).size).toBe(6); expect(Math.min(...centers.map(point => point.x))).toBeLessThan(center.x); expect(Math.max(...centers.map(point => point.x))).toBeGreaterThan(center.x); expect(Math.min(...centers.map(point => point.y))).toBeLessThan(center.y); expect(Math.max(...centers.map(point => point.y))).toBeGreaterThan(center.y); expect(new Set(Array.from({ length: 20 }, (_, index) => p3LandingGroupIndex(index))).size).toBe(6); expect(distance(p3LandingPosition(0, center), p3LandingPosition(2, center))).toBeLessThan(8); expect(p3LandingSoakPositions(0, center)).toEqual(p3LandingSoakPositions(2, center)); expect(distance(p3LandingGroupCenter(0, center), p3LandingSoakPositions(0, center)[0])).toBeGreaterThan(24); expect(p3LandingSoakPositions(0, center)).not.toEqual(p3LandingSoakPositions(3, center)) })
  ...
  it('keeps struck P2 orbs orbiting for ten seconds before a one-second inward return', () => { expect(p2OrbReturnState(-1).phase).toBe('inactive'); expect(p2OrbReturnState(9.99)).toEqual({ phase: 'orbiting', radius: 82 }); expect(p2OrbReturnState(10).phase).toBe('returning'); expect(p2OrbReturnState(10.5).radius).toBeCloseTo(61.5); expect(p2OrbReturnState(11)).toEqual({ phase: 'done', radius: 0 }) })
  it('provides four deterministic P2 return-orb collision positions', () => { const center = { x: 480, y: 270 }; const positions = p2ReturningOrbPositions(10.5, 2, 30, center); expect(positions).toHaveLength(4); positions.forEach(position => expect(distance(position, center)).toBeCloseTo(61.5)); expect(p2ReturningOrbPositions(11, 2, 30, center)).toEqual([]) })
  ...
  it('moves NPC Splinters backward when their side step would still clip the stack', () => {
    const center = { x: 480, y: 270 }
