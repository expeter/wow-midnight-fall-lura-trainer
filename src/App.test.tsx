...
function renderRaidPlan() {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Raid plan' }))
  return view
}
...
describe('player menu', () => {
  ...
  it('persists the selected Phase 1 rune-panel orientation', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'HUD' }))
  ...
  it('derives crystal duty from the phase roster and starts with a countdown', async () => { const user = userEvent.setup(); render(<App />); await user.selectOptions(screen.getByLabelText(/character to play/i), '1'); await user.click(screen.getByRole('button', { name: 'Raid plan' })); expect(screen.getByLabelText(/intermission crystal 1/i)).toHaveValue('1'); await user.click(screen.getByRole('button', { name: /enter arena/i })); expect(screen.getByText(/Get ready/i)).toBeInTheDocument(); expect(screen.getByText('3')).toBeInTheDocument(); expect(screen.getByText(/CRYSTAL CARRIER/i)).toBeInTheDocument(); expect(screen.getByText(/You received a crystal · Intermission/i)).toBeInTheDocument(); expect(screen.getByText('The veil shudders with every step.')).toBeInTheDocument(); expect(screen.queryByText(/MAIN ABILITY READY/i)).not.toBeInTheDocument(); expect(screen.getByText('Points')).toBeInTheDocument(); expect(document.querySelector('.hud')).not.toBeInTheDocument() })
  it('shows the character control and difficulty choices', () => { render(<App />); expect(screen.getByLabelText(/character to play/i)).toBeInTheDocument(); expect(screen.getByRole('button', { name: 'easy' })).toBeInTheDocument(); expect(screen.getByRole('button', { name: 'hard' })).toBeInTheDocument() })
  ...
  it('copies the build identifier and links GitHub, changelog, and issue filing', async () => { const user = userEvent.setup(); const writeText = vi.fn().mockResolvedValue(undefined); Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText } }); render(<App />); const build = screen.getByLabelText(/build information/i); expect(build).toHaveTextContent(/v0\.6\.0 · (?:unknown|[0-9a-f]{7}) · \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC/); fireEvent.click(screen.getByRole('button', { name: /v0\.6\.0.*copy/i })); await waitFor(() => expect(writeText).toHaveBeenCalledWith(expect.stringMatching(/^v0\.6\.0 · (?:unknown|[0-9a-f]{7}) · \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC$/))); expect(screen.getByRole('link', { name: /^github/i })).toHaveAttribute('href', 'https://github.com/expeter/wow-midnight-fall-lura-trainer'); expect(screen.getByRole('link', { name: /changelog/i })).toHaveAttribute('href', 'https://github.com/expeter/wow-midnight-fall-lura-trainer/blob/main/CHANGELOG.md'); expect(screen.getByRole('link', { name: /file an issue/i })).toHaveAttribute('href', 'https://github.com/expeter/wow-midnight-fall-lura-trainer/issues/new/choose'); await user.click(screen.getByRole('button', { name: /enter arena/i })); expect(screen.getByLabelText(/build information/i)).toBeInTheDocument() })
  it('offers to load a newly deployed Git revision and previews recent changes', async () => { vi.mocked(fetch).mockResolvedValueOnce({ ok: true, json: async () => ({ version: '0.4.0', revision: 'abcdef1', builtAt: new Date().toISOString() }) } as Response); const user = userEvent.setup(); render(<App />); const alert = await screen.findByRole('alert'); expect(alert).toHaveTextContent(/new trainer version available.*abcdef1/i); expect(within(alert).getByRole('button', { name: /load new version/i })).toBeInTheDocument(); await user.click(within(alert).getByText(/what’s changed/i)); expect(alert).toHaveTextContent(/live public wipe activity/i); expect(within(alert).getByRole('link', { name: /full changelog/i })).toHaveAttribute('href', 'https://github.com/expeter/wow-midnight-fall-lura-trainer/blob/main/CHANGELOG.md'); await user.click(within(alert).getByRole('button', { name: /later/i })); expect(screen.queryByRole('alert')).not.toBeInTheDocument() })
  ...
  it('previews a clearly marked full completion screen without unlocking achievements', async () => { const user = userEvent.setup(); render(<App />); expect(screen.queryByRole('button', { name: /preview final screen/i })).not.toBeInTheDocument(); await user.click(screen.getByRole('button', { name: 'test' })); await user.click(screen.getByRole('button', { name: /preview final screen/i })); expect(screen.getByText('RESULT SCREEN PREVIEW')).toBeInTheDocument(); expect(screen.getByText(/Preview data only/i)).toBeInTheDocument(); expect(screen.getByRole('heading', { name: /L’ura conquered/i })).toBeInTheDocument(); const phases = screen.getByLabelText('Phase results'); expect(phases.querySelectorAll('article')).toHaveLength(5); expect(phases).toHaveTextContent('Phase 1'); expect(phases).toHaveTextContent('990 pts'); expect(phases).toHaveTextContent('Phase contribution −10'); expect(phases).toHaveTextContent('850 pts'); expect(phases).toHaveTextContent('Phase contribution −90'); expect(localStorage.getItem('lura-achievement-collection')).toBeNull() })
  it('brands an offline Run-ID into the compact result without a separate copy action', async () => {
    const writeText = vi.fn().mockResolvedValue(undefined)
    const user = userEvent.setup()
    Object.defineProperty(navigator, 'clipboard', { configurable: true, value: { writeText } })
    render(<App />)
    await user.click(screen.getByRole('button', { name: 'test' }))
    await user.click(screen.getByRole('button', { name: /preview final screen/i }))
    expect(await screen.findByText(/^LURA1-(?:[0-9A-F]{4}-){4}[0-9A-F]{4}$/)).toBeVisible()
    ...
    await waitFor(() => expect(writeText).toHaveBeenCalledWith(expect.stringMatching(/Run-ID: LURA1-(?:[0-9A-F]{4}-){4}[0-9A-F]{4}$/)))
    expect(writeText).not.toHaveBeenCalledWith(expect.stringContaining('Run data:'))
  ...
  it('assigns a name and WoW class color and saves all profiles', async () => { const user = userEvent.setup(); renderRaidPlan(); await user.clear(screen.getByLabelText(/raid position name/i)); await user.type(screen.getByLabelText(/raid position name/i), 'Lunara'); await user.selectOptions(screen.getByLabelText(/player class and color/i), 'mage'); await user.click(screen.getByRole('button', { name: /save layout/i })); const saved = JSON.parse(localStorage.getItem('lura-player-profiles') || '[]'); expect(saved).toHaveLength(20); expect(saved[0]).toMatchObject({ name: 'Lunara', playerClass: 'mage', crystal: false }) })
  it('shows four configurable raid-plan start slots and saves them', async () => { const user = userEvent.setup(); renderRaidPlan(); expect(screen.getAllByRole('button', { name: /start slot/i })).toHaveLength(4); await user.click(screen.getByRole('button', { name: /save layout/i })); expect(JSON.parse(localStorage.getItem('lura-start-slots') || '[]')).toHaveLength(4) })
  it('creates a shareable raid-plan link and supports P pause/resume', async () => { const user = userEvent.setup(); renderRaidPlan(); await user.click(screen.getByRole('button', { name: /copy share link/i })); expect((screen.getByLabelText(/raid plan share code/i) as HTMLInputElement).value).toContain('#raidplan='); await user.click(screen.getByRole('button', { name: /enter arena/i })); await user.keyboard('p'); expect(screen.getByRole('button', { name: /resume/i })).toBeInTheDocument(); await user.keyboard('p'); expect(screen.getByRole('button', { name: /^pause$/i })).toBeInTheDocument() })
  it('configures, persists, and shares the two Heaven’s Lance tanks', async () => {
    const user = userEvent.setup()
    renderRaidPlan()
    const editor = screen.getByRole('region', { name: 'Tank assignments' })
    expect(within(editor).getByRole('heading', { name: 'Two Heaven’s Lance tanks' })).toBeInTheDocument()
    await user.selectOptions(within(editor).getByLabelText('Tank 1'), '4')
    await user.selectOptions(within(editor).getByLabelText('Tank 2'), '7')
    await user.click(screen.getByRole('button', { name: /save layout/i }))
    expect(JSON.parse(localStorage.getItem('lura-tank-assignments') || '[]')).toEqual([4, 7])
    ...
    expect(payload.tankAssignments).toEqual([4, 7])
  })
  it('gives a directly controlled Phase 4 tank the cone role instead of Starsplinter', async () => {
    localStorage.setItem('lura-hud-action-buttons-enabled', 'true')
    const user = userEvent.setup()
    render(<App />)
    await user.click(screen.getByRole('button', { name: /^p4$/i }))
    await user.click(screen.getByRole('button', { name: /enter p4/i }))
    const arena = document.querySelector('.arena-wrap')
    expect(arena).toHaveAttribute('data-tank-role', 'tank-1')
    expect(arena).toHaveAttribute('data-p4-player-splinter-duty', '-1')
    expect(screen.getByLabelText('Phase 4 tank cone')).toHaveTextContent(/You will not receive Starsplinter/i)
    expect(within(screen.getByLabelText('HUD action buttons')).getByRole('button', { name: /Tank cone/i })).toBeInTheDocument()
  })
  it('makes the second controlled Phase 4 tank carry the protection zone without Starsplinter or cone', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /enter p4/i }))
    const arena = document.querySelector('.arena-wrap')
    expect(arena).toHaveAttribute('data-tank-role', 'tank-2')
    expect(arena).toHaveAttribute('data-p4-player-splinter-duty', '-1')
    expect(screen.getByLabelText('Phase 4 protection tank')).toHaveTextContent(/Your position carries the yellow safe zone/i)
    expect(within(screen.getByLabelText('HUD action buttons')).queryByRole('button', { name: /Tank cone/i })).not.toBeInTheDocument()
  })
  ...
  it('orders game start, global Top 3, and the player row before the setup sections', async () => {
    ...
    const assignment = screen.getByRole('group', { name: /character to play/i })
    const combat = screen.getByRole('group', { name: /^combat actions$/i })
    expect(screen.getByText('GAME SETTINGS')).toBeInTheDocument()
    expect(screen.getByText('KEYBOARD SETTINGS')).not.toBeVisible()
    expect(screen.getByText('INTERFACE')).not.toBeVisible()
    ...
    fireEvent.click(within(setupNav).getByRole('button', { name: 'HUD' }))
    expect(screen.getByRole('heading', { name: /hud positions/i })).toBeVisible()
    expect(screen.getByText('Keyboard & mouse controls')).not.toBeVisible()

    fireEvent.click(within(setupNav).getByRole('button', { name: 'Raid plan' }))
    expect(screen.getByText('RAID PLANNING')).toBeInTheDocument()
    expect(screen.getByRole('group', { name: /raid-plan sharing/i })).toBeInTheDocument()
    ...
    fireEvent.click(within(setupNav).getByRole('button', { name: 'Profile' }))
    expect(screen.getByRole('heading', { name: 'My characters' })).toBeVisible()
    expect(screen.queryByText('Verified public rankings')).not.toBeInTheDocument()
  })
  ...
  it('allows an Easy-mode recovery charge and greys it out after use', async () => { localStorage.setItem('lura-tank-assignments', JSON.stringify([18, 19])); const user = userEvent.setup(); render(<App />); await user.click(screen.getByRole('button', { name: 'easy' })); await user.click(screen.getByRole('button', { name: /enter arena/i })); expect(screen.getByTitle('Health potion ready')).toBeInTheDocument(); fireEvent.keyDown(window, { code: 'NumpadDecimal' }); expect(screen.getByTitle('Health potion used until next phase')).toHaveClass('used'); expect(screen.getByTitle('Shield ready')).not.toHaveClass('used') })
  ...
  it('provides six independent crystal selectors beneath each phase plan', () => { renderRaidPlan(); for (const phase of ['Phase 1', 'Intermission', 'Phase 2', 'Phase 3']) { const editor = screen.getByLabelText(`${phase} crystal assignments`); expect(within(editor).getAllByRole('combobox')).toHaveLength(6); expect(within(editor).getByLabelText(`${phase} crystal 1`)).toHaveValue('1') } })
  it('uses the darker petrol class color for Monk assignments', () => { renderRaidPlan(); expect(screen.getByRole('button', { name: 'Move player 13' })).toHaveStyle({ backgroundColor: '#00a98f' }) })
  it('loads, resets, and persists the draggable HUD layout including the castbar', async () => { localStorage.setItem('lura-hud-layout', JSON.stringify({ mechanic: { x: 22, y: 18 }, beam: { x: 72, y: 31 }, crystal: { x: 66, y: 82 } })); const user = userEvent.setup(); render(<App />); await user.click(screen.getByRole('button', { name: 'HUD' })); expect(screen.getByLabelText(/phase 2 hud layout preview/i)).toBeInTheDocument(); expect(screen.getByText(/static p2 view · drag the hud boxes/i)).toBeInTheDocument(); expect(screen.getByRole('button', { name: /move beam in counter/i })).toHaveStyle({ left: '72%', top: '31%' }); expect(screen.getByRole('button', { name: /move player health counter/i })).toHaveStyle({ left: '21%', top: '53%' }); expect(screen.getByRole('button', { name: /move main ability counter/i })).toHaveStyle({ left: '50%', top: '65%' }); expect(screen.getByRole('button', { name: /move action buttons counter/i })).toHaveStyle({ left: '50%', top: '68%' }); await user.click(screen.getByRole('button', { name: /reset counter positions/i })); expect(screen.getByRole('button', { name: /move beam in counter/i })).toHaveStyle({ left: '57%', top: '23%' }); await waitFor(() => { const saved = JSON.parse(localStorage.getItem('lura-hud-layout') || '{}'); expect(saved.castbar).toEqual({ x: 50, y: 65 }); expect(saved.actions).toEqual({ x: 50, y: 68 }) }) })
  it('keeps the optional HUD action buttons off by default and routes enabled clicks through existing actions', async () => { const user = userEvent.setup(); render(<App />); await user.selectOptions(screen.getByLabelText(/character to play/i), '1'); await user.click(screen.getByRole('button', { name: 'HUD' })); const toggle = screen.getByLabelText(/show hud action buttons/i); expect(toggle).not.toBeChecked(); await user.click(toggle); await waitFor(() => expect(localStorage.getItem('lura-hud-action-buttons-enabled')).toBe('true')); await user.click(screen.getByRole('button', { name: /enter arena/i })); const actions = screen.getByLabelText(/hud action buttons/i); expect(actions).toHaveStyle({ left: '50%', top: '68%' }); await user.click(within(actions).getByRole('button', { name: /main ability/i })); expect(screen.getByText(/MAIN ABILITY · 1\.0s/i)).toBeInTheDocument(); await user.click(within(actions).getByRole('button', { name: /health potion/i })); expect(screen.getByTitle('Health potion used until next phase')).toHaveClass('used'); expect(within(actions).getByRole('button', { name: /health potion/i })).toBeDisabled(); await user.click(within(actions).getByRole('button', { name: /crystal drop/i })); expect(screen.getByText(/PICK UP IN/i)).toBeInTheDocument(); expect(within(actions).getByRole('button', { name: /crystal drop/i })).toBeDisabled() })
  it('enables live activity messages by default and persists the HUD opt-out', async () => { const user = userEvent.setup(); render(<App />); await user.click(screen.getByRole('button', { name: 'HUD' })); const toggle = screen.getByLabelText(/show live activity notifications/i); expect(toggle).toBeChecked(); await user.click(toggle); await waitFor(() => expect(localStorage.getItem('lura-live-activity-notifications-enabled')).toBe('false')) })
  ...
  it('changes Phase 2 crystal ownership without moving any planned player position', async () => { const savedSoaks = Array.from({ length: 20 }, (_, index) => ({ x: 100 + index * 11, y: 120 + index * 7 })); const savedSpreads = Array.from({ length: 20 }, (_, index) => ({ x: 140 + index * 9, y: 160 + index * 5 })); localStorage.setItem('lura-p2-player-positions', JSON.stringify(savedSoaks)); localStorage.setItem('lura-p2-spread-positions', JSON.stringify(savedSpreads)); const user = userEvent.setup(); renderRaidPlan(); await user.click(screen.getByRole('button', { name: /save layout/i })); const loadedSoaks = JSON.parse(localStorage.getItem('lura-p2-player-positions') || '[]'); const loadedSpreads = JSON.parse(localStorage.getItem('lura-p2-spread-positions') || '[]'); await user.selectOptions(screen.getByLabelText(/phase 2 crystal 1/i), '0'); await user.click(screen.getByRole('button', { name: /layout saved/i })); expect(JSON.parse(localStorage.getItem('lura-p2-player-positions') || '[]')).toEqual(loadedSoaks); expect(JSON.parse(localStorage.getItem('lura-p2-spread-positions') || '[]')).toEqual(loadedSpreads); expect(JSON.parse(localStorage.getItem('lura-p2-crystal-assignments') || '[]')).toContain(0); expect(JSON.parse(localStorage.getItem('lura-intermission-crystal-assignments') || '[]')).not.toContain(0) })
