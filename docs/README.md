...
| `FR-046` | Backlog | Add persistent long-term achievement counters without overcrowding the ledger: **The Impossible** for five consecutive flawless full runs on Normal, a separate Hard-mode five-run counterpart, and phase-clear milestones at 10, 50, and 100 total clears (“Can’t Get Enough”). A failed full run resets only the matching difficulty streak; direct phase practice neither advances nor resets it. Full-run streak tracking currently begins with Intermission and will move to P1 when P1 becomes the encounter entry. |
