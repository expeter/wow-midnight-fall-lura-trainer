...
describe('Intermission game rules', () => {
  ...
  it('starts the struck-orb glow one second earlier without moving the inward return', () => {
    ...
    expect(p2OrbReturnState(11.99)).toEqual({ phase: 'orbiting', radius: 82 })
    expect(p2OrbReturnState(12)).toEqual({ phase: 'charging', radius: 82 })
  ...
  it('keeps each resolved P2 group moving counterclockwise before all four shoot inward together', () => {
    ...
    const returning = p2ReturningOrbPositions(14.5, 2, .8, center)
    expect(returning).toHaveLength(4)
    returning.forEach(position => expect(distance(position, center)).toBeCloseTo(61.5))
    expect(p2ReturningOrbPositions(15, 2, .9, center)).toEqual([])
  })
  ...
  it('phase-gates the P2 return counter and anchors the P4 knockup at the north stack', () => {
    ...
    expect(p2PhaseTransitionCountdown('p2-wait', 3, 14.9)).toBeNull()
    expect(p2PhaseTransitionCountdown('p2-wait', 3, 15)).toBe(8)
