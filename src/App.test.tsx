...
describe('player menu', () => {
  ...
  it('loads, resets, and persists the draggable HUD layout including the castbar', async () => { localStorage.setItem('lura-hud-layout', JSON.stringify({ mechanic: { x: 22, y: 18 }, beam: { x: 72, y: 31 }, crystal: { x: 66, y: 82 } })); const user = userEvent.setup(); render(<App />); await user.click(screen.getByRole('button', { name: 'HUD' })); expect(screen.getByLabelText(/phase 2 hud layout preview/i)).toBeInTheDocument(); expect(screen.getByText(/static p2 view · drag the hud boxes/i)).toBeInTheDocument(); expect(screen.getByRole('button', { name: /move beam in counter/i })).toHaveStyle({ left: '72%', top: '31%' }); expect(screen.getByRole('button', { name: /move player health counter/i })).toHaveStyle({ left: '21%', top: '53%' }); expect(screen.getByRole('button', { name: /move main ability counter/i })).toHaveStyle({ left: '50%', top: '65%' }); await user.click(screen.getByRole('button', { name: /reset counter positions/i })); expect(screen.getByRole('button', { name: /move beam in counter/i })).toHaveStyle({ left: '57%', top: '23%' }); await waitFor(() => expect(JSON.parse(localStorage.getItem('lura-hud-layout') || '{}').castbar).toEqual({ x: 50, y: 65 })) })
  it('keeps the optional HUD action buttons off by default and routes enabled clicks through existing actions', async () => { const user = userEvent.setup(); render(<App />); await user.selectOptions(screen.getByLabelText(/character to play/i), '1'); await user.click(screen.getByRole('button', { name: 'HUD' })); const toggle = screen.getByLabelText(/show hud action buttons/i); expect(toggle).not.toBeChecked(); await user.click(toggle); await waitFor(() => expect(localStorage.getItem('lura-hud-action-buttons-enabled')).toBe('true')); await user.click(screen.getByRole('button', { name: /enter arena/i })); const actions = screen.getByLabelText(/hud action buttons/i); expect(actions).toHaveStyle({ left: '50%', top: '65%' }); await user.click(within(actions).getByRole('button', { name: /main ability/i })); expect(screen.getByText(/MAIN ABILITY · 1\.0s/i)).toBeInTheDocument(); await user.click(within(actions).getByRole('button', { name: /health potion/i })); expect(screen.getByTitle('Health potion used until next phase')).toHaveClass('used'); expect(within(actions).getByRole('button', { name: /health potion/i })).toBeDisabled(); await user.click(within(actions).getByRole('button', { name: /crystal drop/i })); expect(screen.getByText(/PICK UP IN/i)).toBeInTheDocument(); expect(within(actions).getByRole('button', { name: /crystal drop/i })).toBeDisabled() })
