...
describe('player menu', () => {
  ...
  it('provides, saves, and shares the Phase 3 players and movable bosses', async () => { const user = userEvent.setup(); renderRaidPlan(); expect(screen.getByLabelText(/phase 3 initial position map/i)).toBeInTheDocument(); expect(screen.getAllByRole('button', { name: /move p3 player/i })).toHaveLength(20); expect(screen.getAllByRole('button', { name: /move p3 .* boss/i })).toHaveLength(2); await user.click(screen.getByRole('button', { name: /save layout/i })); expect(JSON.parse(localStorage.getItem('lura-p3-player-positions') || '[]')).toHaveLength(20); expect(JSON.parse(localStorage.getItem('lura-p3-boss-positions') || '[]')).toHaveLength(2); await user.click(screen.getByRole('button', { name: /copy share link/i })); const link = (screen.getByLabelText(/raid plan share code/i) as HTMLInputElement).value; const payload = JSON.parse(decodeURIComponent(atob(link.split('#raidplan=')[1]))); expect(payload.p3Positions).toHaveLength(20); expect(payload.p3BossPositions).toHaveLength(2) })
  ...
  it('moves newly assigned Phase 2 crystal carriers onto an inner spread spot', async () => { const user = userEvent.setup(); renderRaidPlan(); await user.selectOptions(screen.getByLabelText(/phase 2 crystal 1/i), '0'); await user.click(screen.getByRole('button', { name: /save layout/i })); const [spot] = JSON.parse(localStorage.getItem('lura-p2-spread-positions') || '[]'); expect(Math.hypot(spot.x - 480, spot.y - 270)).toBeLessThanOrEqual(46.01); expect(JSON.parse(localStorage.getItem('lura-p2-crystal-assignments') || '[]')).toContain(0); expect(JSON.parse(localStorage.getItem('lura-intermission-crystal-assignments') || '[]')).not.toContain(0) })
