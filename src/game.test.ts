...
describe('Intermission game rules', () => {
  ...
  it('treats a Starsplinter hitting a carrying crystal NPC as a wipe target', () => {
    const npcPositions = Array.from({ length: 19 }, (_, index) => ({ x: 100 + index, y: 100 }))
    npcPositions[4] = { x: 120, y: 100 }
  ...
  it('starts Phase 4 with a fresh health pool and awards each newly crossed ten-percent tier', () => {
    expect(p4StartingBossState()).toEqual({ health: 100, playerDamage: 0 })
  ...
  it('adds five percent pre-Phase-4 health without changing the Phase 3 clear flow', () => {
    expect(preP4BossHealth(99)).toBeCloseTo(5.714, 3)
    expect(preP4BossHealth(100)).toBeCloseTo(4.762, 3)
    expect(preP4BossHealth(104.5)).toBeCloseTo(.476, 3)
    expect(preP4BossHealth(105)).toBe(0)
    expect(preP4BossHealth(120)).toBe(0)
    expect(preP4BossHealth(105, true)).toBe(3)
    expect(preP4BossHealth(120, true)).toBe(3)
    expect(shouldTriggerP3EarlyClear('p3-light-pools', 105, 2)).toBe(true)
    expect(shouldTriggerP3EarlyClear('p3-archangel', 105, 2)).toBe(true)
    expect(shouldTriggerP3EarlyClear('p3-light-pools', 105, 1)).toBe(false)
    expect(shouldTriggerP3EarlyClear('p3-light-pools', 104.5, 2)).toBe(false)
    expect(shouldTriggerP3EarlyClear('p3-countdown', 105, 2)).toBe(false)
    expect(shouldTriggerP3EarlyClear('p3-sector-move', 105, 2)).toBe(false)
    expect(shouldTriggerP3EarlyClear('p4-cycle', 105, 2)).toBe(false)
  })
  ...
  it('allows movement against most of the pull but overwhelms walking at full force', () => { const bounds = { minX: 0, maxX: 1000, minY: 0, maxY: 1000 }; const start = { x: 600, y: 500 }; const center = { x: 500, y: 500 }; expect(moveWithIncreasingPull(start, new Set(['w']), 136, 1, { x: 1, y: 0 }, bounds, center, .75).x).toBeGreaterThan(start.x); expect(moveWithIncreasingPull(start, new Set(['w']), 136, 1, { x: 1, y: 0 }, bounds, center, 1).x).toBeLessThan(start.x) })
  ...
  it('cancels a rendered six-second expiry once pickup resets the authoritative ground age', () => {
    expect(groundCrystalHasExpired(6, 6)).toBe(true)
    expect(groundCrystalHasExpired(6, 0)).toBe(false)
  })
  ...
  it('limits the legacy annulus void check to live Intermission mechanics', () => {
    ...
    expect(shouldCheckIntermissionVoid('p2-wait')).toBe(false)
    expect(shouldCheckIntermissionVoid('p3-flight')).toBe(false)
  })
  it('shows a carrier indicator exactly while the player carries the crystal', () => {
    const p2Carrier = { phaseOne: false, p1AssignedAndCollected: false, p1WrongCrystalHeld: false, assigned: true, spent: false, dropped: false, phaseFour: false }
    expect(playerCarriesCrystal(p2Carrier)).toBe(true)
    expect(playerCarriesCrystal({ ...p2Carrier, dropped: true })).toBe(false)
    expect(playerCarriesCrystal({ ...p2Carrier, spent: true })).toBe(false)
    expect(playerCarriesCrystal({ ...p2Carrier, phaseOne: true, assigned: false, p1AssignedAndCollected: true })).toBe(true)
  })
  ...
  it('treats a P2 crystal touch as owned for a carrier and recoverably wrong otherwise', () => { expect(p2CrystalTouchResolution(true)).toBe('assigned'); expect(p2CrystalTouchResolution(false)).toBe('wrong-recoverable') })
  it('assigns every P2 beam set to four deterministic unique non-crystal players', () => {
    const crystals = [1, 4, 7, 10, 13, 16]
    const first = p2BeamPlayers(1234, 1, crystals)
    const repeated = p2BeamPlayers(1234, 1, crystals)
    const second = p2BeamPlayers(1234, 2, crystals)
    expect(first).toHaveLength(4)
    expect(new Set(first).size).toBe(4)
    expect(first.some(index => crystals.includes(index))).toBe(false)
    expect(repeated).toEqual(first)
    expect(second).not.toEqual(first)
  })
  it('targets each of the twelve continuously orbiting P2 orbs once across three beam sets', () => {
    let resolved: number[] = []
    for (let cycle = 1; cycle <= 3; cycle += 1) {
      const impactAngle = .37 + cycle * P2_BEAM_SECONDS * .18
      const targets = p2BeamTargetOrbIndices(resolved, impactAngle)
      expect(targets).toHaveLength(4)
      expect(new Set(targets).size).toBe(4)
      expect(targets.some(index => resolved.includes(index))).toBe(false)
      resolved = [...resolved, ...targets]
    }
    expect(new Set(resolved)).toEqual(new Set(Array.from({ length: 12 }, (_, index) => index)))
  })
  it('validates a selected P2 player beam against its assigned rendered orb ray', () => {
    const center = { x: 480, y: 270 }
    const orbitAngle = .41
    const orbIndex = p2BeamTargetOrbIndices([], orbitAngle)[0]
    const aim = p2BeamAimPosition(orbIndex, orbitAngle, center)
    const orb = p2OrbPosition(orbIndex, orbitAngle, center)
    expect(p2BeamHitsAssignedOrb(aim, orb, center)).toBe(true)
    expect(p2BeamHitsAssignedOrb({ x: center.x - (aim.x - center.x), y: center.y - (aim.y - center.y) }, orb, center)).toBe(false)
  })
  it('pins public P2 beam rendering to the four raid-marker axes', () => {
    const aimedAngle = .37
    expect([0, 1, 2, 3].map(index => p2BeamRenderAngle(index, aimedAngle, true))).toEqual([
      0,
      Math.PI / 2,
      Math.PI,
      Math.PI * 1.5,
    ])
    expect(p2BeamRenderAngle(2, aimedAngle, false)).toBe(aimedAngle)
  })
  ...
  it('ends Normal when accumulated penalties exhaust all points', () => { expect(scoreExhaustionWipeReason('normal', 0)).toBe('Points reached zero'); expect(scoreExhaustionWipeReason('normal', 1)).toBeNull(); expect(scoreExhaustionWipeReason('easy', 0)).toBeNull(); expect(scoreExhaustionWipeReason('test', 0)).toBeNull() })
  ...
  it('accepts player or NPC occupancy in either yellow landing Soak and visibly damages a player outside', () => { const soaks = [{ x: 10, y: 10 }, { x: 40, y: 10 }]; expect(p3LandingSoakOccupied(soaks, { x: 100, y: 100 }, [{ x: 40, y: 10 }])).toBe(true); expect(p3LandingSoakOccupied(soaks, { x: 10, y: 10 }, [])).toBe(true); expect(p3LandingSoakOccupied(soaks, { x: 100, y: 100 }, [{ x: 100, y: 90 }])).toBe(false); expect(p3LandingHealthRate(false)).toBe(-18); expect(p3LandingHealthRate(true)).toBeGreaterThan(0) })
  ...
  it('lets both P3 raid halves finish at the exact shared P4 north stack', () => {
    const center = { x: 480, y: 270 }
    const stack = p4TransitionStartPosition(center)
    expect(constrainP3NpcTargetToSide(stack, -1, center, 26, 'p3-sector-move', 2)).toEqual(stack)
    expect(constrainP3NpcTargetToSide(stack, 1, center, 26, 'p3-sector-move', 2)).toEqual(stack)
    expect(constrainP3NpcTargetToSide(stack, -1, center, 26, 'p3-sector-move', 1).x).toBe(center.x - 26)
  })
  ...
  it('places crystal carriers outside Phase 3 Soaks while their lights cover every possible pool', () => {
    ...
    for (const side of [-1, 1] as const) {
      ...
      expect(targets.every(target => pools.every(pool => distance(target, pool) >= P3_POOL_RADIUS + P3_POOL_CRYSTAL_CLEARANCE))).toBe(true)
  ...
  it('never connects Phase 3 Stars orbs beyond the local beam range', () => {
    const center = { x: 480, y: 270 }
    const orbs = [{ x: 300, y: 220 }, { x: 300 + P3_RUNE_CONNECTION_DISTANCE, y: 220 }, { x: 300 + P3_RUNE_CONNECTION_DISTANCE + .01, y: 260 }]
    const edges = p3RuneEdges(-1, center, 1, orbs)
    expect(edges).toContainEqual([0, 1])
    expect(edges).not.toContainEqual([0, 2])
    expect(edges.every(([from, to]) => distance(orbs[from], orbs[to]) <= P3_RUNE_CONNECTION_DISTANCE)).toBe(true)
  })
  it('keeps P3 Stars beams local, non-crossing, and below three links per orb', () => {
    const center = { x: 480, y: 270 }
    for (let layout = 1; layout <= 20; layout += 1) {
      randomizeP3PoolLayout(layout)
      for (const side of [-1, 1] as const) {
        for (const round of [1, 2]) {
          for (let cycle = 0; cycle < 8; cycle += 1) {
            const orbs = p3RuneOrbs(side, center, round, cycle)
            const edges = p3RuneEdges(side, center, round, orbs)
            const degree = orbs.map((_, index) => edges.filter(edge => edge.includes(index)).length)
            const crossing = edges.some(([a, b], edgeIndex) => edges.slice(edgeIndex + 1).some(([c, d]) =>
              a !== c && a !== d && b !== c && b !== d
              && ((orbs[b].x - orbs[a].x) * (orbs[c].y - orbs[a].y) - (orbs[b].y - orbs[a].y) * (orbs[c].x - orbs[a].x))
                * ((orbs[b].x - orbs[a].x) * (orbs[d].y - orbs[a].y) - (orbs[b].y - orbs[a].y) * (orbs[d].x - orbs[a].x)) < 0
              && ((orbs[d].x - orbs[c].x) * (orbs[a].y - orbs[c].y) - (orbs[d].y - orbs[c].y) * (orbs[a].x - orbs[c].x))
                * ((orbs[d].x - orbs[c].x) * (orbs[b].y - orbs[c].y) - (orbs[d].y - orbs[c].y) * (orbs[b].x - orbs[c].x)) < 0
            ))
            expect(edges.length, `layout ${layout}, side ${side}, round ${round}, cycle ${cycle}`).toBeGreaterThan(8)
            expect(edges.every(([from, to]) => distance(orbs[from], orbs[to]) <= P3_RUNE_CONNECTION_DISTANCE)).toBe(true)
            expect(Math.max(...degree)).toBeLessThanOrEqual(3)
            expect(crossing).toBe(false)
          }
  ...
  it('changes the P3 Stars render-cache identity with each randomized Soak layout', () => {
    randomizeP3PoolLayout(101)
    expect(p3PoolLayoutId()).toBe(101)
    randomizeP3PoolLayout(202)
    expect(p3PoolLayoutId()).toBe(202)
  })
  ...
  it('keeps the P2 sequence on its configured thirty-second cross-beam cadence', () => {
    ...
    expect(P2_BEAM_SECONDS + P2_ORB_RETURN_SECONDS).toBe(21)
  ...
  it('starts the fixed P2 pull after every beam instead of waiting for crystal pickup', () => {
    expect(p2PostBeamEvent()).toBe('p2-pull')
    expect(P2_PULL_SECONDS).toBe(5)
    expect(P2_SPREAD_SECONDS).toBe(5)
    expect(P2_FETCH_SECONDS).toBe(6)
  })
  ...
  it('starts the struck-orb glow one second earlier without moving the inward return', () => {
    expect(p2OrbReturnState(-1).phase).toBe('inactive')
    expect(P2_ORB_GLOW_LEAD_SECONDS).toBe(1)
    ...
    expect(p2OrbReturnState(14)).toEqual({ phase: 'charging', radius: 82 })
    expect(p2OrbReturnState(15).phase).toBe('returning')
    expect(p2OrbReturnState(15.5).radius).toBeCloseTo(61.5)
    expect(p2OrbReturnState(16)).toEqual({ phase: 'done', radius: 0 })
  })
  it('turns each struck group of the twelve-orb P2 roster yellow without creating overlapping copies', () => {
    const center = { x: 480, y: 270 }
    const orbitAngle = .42
    const roster = Array.from({ length: 12 }, (_, index) => p2OrbPosition(index, orbitAngle, center))
    expect(new Set(roster.map(point => `${point.x.toFixed(5)},${point.y.toFixed(5)}`)).size).toBe(12)
    for (let cycle = 1; cycle <= 3; cycle++) {
      const firstIndex = (cycle - 1) * 4
      const yellow = p2ReturningOrbPositions(13, cycle, orbitAngle, center)
      expect(yellow).toEqual(roster.slice(firstIndex, firstIndex + 4))
      const purple = roster.filter((_, index) => index < firstIndex || index >= firstIndex + 4)
      yellow.forEach(returner => purple.forEach(orb => expect(distance(returner, orb)).toBeGreaterThan(20)))
    }
  })
  it('keeps each resolved P2 group moving counterclockwise before all four shoot inward together', () => {
    const center = { x: 480, y: 270 }
    const before = p2ReturningOrbPositions(12, 2, .7, center)
    const after = p2ReturningOrbPositions(12.02, 2, .7024, center)
    before.forEach((point, index) => {
      const next = after[index]
      const cross = (point.x - center.x) * (next.y - center.y) - (point.y - center.y) * (next.x - center.x)
      expect(cross).toBeGreaterThan(0)
    })
    const returning = p2ReturningOrbPositions(15.5, 2, .8, center)
    expect(returning).toHaveLength(4)
    returning.forEach(position => expect(distance(position, center)).toBeCloseTo(61.5))
    expect(p2ReturningOrbPositions(16, 2, .9, center)).toEqual([])
  })
  ...
  it('provides enough first-sector movement for either standard post-Archangel relocation', () => {
    const center = { x: 480, y: 270 }
    const speed = p3SectorMovementSpeed(8)
    expect(speed).toBe(36)
    for (const side of [-1, 1] as const) {
      const start = p3ArchangelStackPosition(side, center, 1)
      const initial = { x: center.x + side * 75, y: center.y + 130 }
      const next = p3AssignmentForRound(initial, center, 2)
      expect(distance(start, next)).toBeLessThanOrEqual(speed * P3_SECTOR_MOVE_SECONDS)
    }
  })
  ...
  it('phase-gates the P2 return counter and anchors the P4 knockup at the north stack', () => {
    ...
    expect(p2PhaseTransitionCountdown('p2-wait', 2, 15)).toBeNull()
    expect(p2PhaseTransitionCountdown('p2-wait', 3, 15.9)).toBeNull()
    expect(p2PhaseTransitionCountdown('p2-wait', 3, 16)).toBe(7)
    expect(p2PhaseTransitionCountdown('p2-wait', 3, 22.5)).toBe(.5)
    expect(p2PhaseTransitionCountdown('p3-flight', 3, 22.5)).toBeNull()
    expect(p2FinalRegroupActive('p2-wait', 3, 15.9)).toBe(false)
    expect(p2FinalRegroupActive('p2-wait', 3, 16)).toBe(true)
    expect(p2FinalRegroupActive('p2-wait', 2, 22.5)).toBe(false)
    expect(p4TransitionStartPosition(center)).toEqual(p4StackPosition(1, center))
  })
  it('flies from an individual P2 circle without snapping through the center', () => {
    const circle = { x: 445, y: 304 }
    const landing = { x: 410, y: 420 }
    expect(p3FlightPosition(circle, landing, 0)).toEqual(circle)
    const halfway = p3FlightPosition(circle, landing, P3_FLIGHT_SECONDS / 2)
    expect(halfway.x).toBeLessThan(circle.x)
    expect(halfway.y).toBeGreaterThan(circle.y)
    expect(p3FlightPosition(circle, landing, P3_FLIGHT_SECONDS)).toEqual(landing)
  })
  ...
  it('makes an NPC Phase 4 Starsplinter lethal only to the controlled player', () => {
    ...
    expect(p4RenderedNpcSplinterHitsPlayer(rendered, 0, fallback, 0, { x: 30, y: 10 })).toBe(true)
    expect(p4RenderedNpcSplinterHitsPlayer(rendered, 0, fallback, 0, { x: 30, y: 18 })).toBe(false)
  })
  ...
  it('keeps the second and third P4 Splinter NPCs stacked until the first Splinter detonates', () => {
    const center = { x: 480, y: 270 }
    const stack = p4StackPosition(1, center)
    for (const ordinal of [1, 2]) {
      const wait = P4_SPLINTER_DETONATION_SECONDS - ordinal * P4_SPLINTER_INTERVAL_SECONDS + P4_SPLINTER_POST_DETONATION_HOLD_SECONDS
      expect(p4NpcSplinterMovementAge(ordinal, wait - .01)).toBe(0)
      expect(p4NpcSplinterPosition(stack, center, ordinal, wait - .01, 0)).toEqual(stack)
      expect(p4NpcSplinterPosition(stack, center, ordinal, wait - P4_SPLINTER_POST_DETONATION_HOLD_SECONDS / 2, 0)).toEqual(stack)
      expect(p4NpcSplinterMovementAge(ordinal, wait + .25)).toBeCloseTo(.25)
      expect(distance(p4NpcSplinterPosition(stack, center, ordinal, wait + .25, 0), stack)).toBeGreaterThan(0)
      expect(distance(p4NpcSplinterPosition(stack, center, ordinal, P4_SPLINTER_DETONATION_SECONDS, 0), stack)).toBeGreaterThan(10)
    }
    expect(p4NpcSplinterOutwardSeconds(1)).toBeCloseTo(.8)
    expect(p4NpcSplinterOutwardSeconds(2)).toBeCloseTo(1.35)
  })
  ...
  it('holds the Phase 4 tank clear of a Splinter beam and resumes its route when it clears', () => {
    const stack = { x: 100, y: 100 }
    const current = { x: 114, y: 105 }
    const desired = { x: 112, y: 100 }
    const beam = [{ origin: { x: 105, y: 100 }, rotation: 0 }]
    expect(p4TankAvoidSplinters(current, desired, stack, beam)).toEqual(current)
    expect(p4TankAvoidSplinters(current, desired, stack, [])).toEqual(desired)
