...
test('enters Phase 3 directly in non-blocking Test mode', async ({ page }) => {
  ...
  await expect(page.getByText('Run to your assigned boss.')).toBeVisible({ timeout: MECHANIC_TIMEOUT })
  await page.keyboard.down('w')
  await expect(page.getByText('Stand in the light and drain pools.')).toBeVisible({ timeout: MECHANIC_TIMEOUT })
  await page.keyboard.up('w')
  await expect(page.getByText(/first twelve-orb lattice forms/i)).toBeVisible()
  await expect(page.getByRole('alert')).toHaveCount(0)
})
