...
describe('player menu', () => {
  ...
  it('previews a clearly marked full completion screen without unlocking achievements', async () => { const user = userEvent.setup(); render(<App />); expect(screen.queryByRole('button', { name: /preview final screen/i })).not.toBeInTheDocument(); await user.click(screen.getByRole('button', { name: 'test' })); await user.click(screen.getByRole('button', { name: /preview final screen/i })); expect(screen.getByText('RESULT SCREEN PREVIEW')).toBeInTheDocument(); expect(screen.getByText(/Preview data only/i)).toBeInTheDocument(); expect(screen.getByRole('heading', { name: /L’ura conquered/i })).toBeInTheDocument(); expect(screen.getByLabelText('Phase results').querySelectorAll('article')).toHaveLength(5); expect(screen.getByLabelText('Phase results')).toHaveTextContent('Phase 1'); expect(localStorage.getItem('lura-achievement-collection')).toBeNull() })
